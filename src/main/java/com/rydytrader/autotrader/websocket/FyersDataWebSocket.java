...
public class FyersDataWebSocket extends WebSocketClient {
    ...
    /** HSM feed mode after auth. FULL (mode byte 70 / ASCII 'F') → dt=85 frames
     *  carrying LTP + ATP + volume + LTT + EFT + prev-close + session vol/turnover
     *  — everything we need for VWAP and bar building. LITE (mode byte 76 / 'L')
     *  → dt=76 frames with LTP only. DEFAULT → don't send a mode-change message,
     *  accept whatever HSM defaults to (historically LITE, so avoid this). */
    public enum Mode { FULL, LITE, DEFAULT }
        ...
        switch (respType) {
            case 1: // Auth response
                ...
                if (ack >= 0) {
                    ...
                    // Step 3: Set feed mode. HSM defaults to LITE (dt=76, LTP-only)
                    // if we don't send a mode-change. For VWAP / bar building we
                    // need FULL (dt=85, LTP + ATP + volume + LTT + EFT + session
                    // vol/turnover). Mode.DEFAULT skips the switch.
                    if (mode == Mode.LITE) {
