...
public class HsmBinaryParser {
    ...
    /** Build lite mode switch message (req_type=12). Mode byte 76 = ASCII 'L'.
     *  LITE frames (dt=76) carry LTP only — no ATP, no volume, no OHLC. */
    public static byte[] buildLiteModeMessage(int channelNum) {
        return buildModeChangeMessage(channelNum, (byte) 76);
    }

    /** Build full mode switch message (req_type=12). Mode byte 70 = ASCII 'F'.
     *  FULL frames (dt=85) carry LTP + ATP + volume + LTT + EFT + prev-close
     *  + session volume/turnover — everything needed for VWAP and bar building.
     *  HSM defaults to LITE after connect; call this to get FULL frames. */
    public static byte[] buildFullModeMessage(int channelNum) {
        return buildModeChangeMessage(channelNum, (byte) 70);
    }

    private static byte[] buildModeChangeMessage(int channelNum, byte modeByte) {
    ...
    private static RawTick buildTick(SymbolMeta meta, Map<String, String> hsmToFyersSymbol) {
        ...
        Integer atpRaw = meta.rawValues.get("avg_trade_price");
        if (atpRaw != null) tick.atp = atpRaw / divisor;
